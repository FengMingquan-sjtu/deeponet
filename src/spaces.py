...
class FiniteChebyshev:
    ...
    def random(self, n):
        # generate an array of random paramters
        # n is number of 't', self.N is number of features(coefficients)(the order of polynomial)
        return 2 * self.M * np.random.rand(n, self.N) - self.M

    def eval_u_one(self, a, x):
        # a is the coefficent of cheby polynomial, generated by self.random()
...
class GRF(object):
    ...
    def eval_u_one(self, y, x):
        ...
        """
        # x: sensors
        # y: features, generated by self.random()
        # self.x: fixed points of GRF, used to interpolate.
        if self.interp == "linear":
            return np.interp(x, np.ravel(self.x), y) 
            ##interp(x, xp, fp): given data (xp, f(xp)), regress f(x).
            ## i.e. given (self.x, u(self.x)=y), regress u(x).
            ## since self.x is a simple linespace, the y contains all information of GRF.
            ## Notice for linear interp, the output u(x) can be viewed as linear combination of y.
            ## that is, output u(x) is represented by basis expension.
